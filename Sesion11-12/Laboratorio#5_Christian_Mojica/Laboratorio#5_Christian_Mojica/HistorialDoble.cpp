...
namespace EIF201 {
	...
	HistorialDoble::~HistorialDoble() {
		...
		cabeza = nullptr;
		cola = nullptr;
		cout << "[HisotrialDoble Destruido]" << endl;
	}

	void HistorialDoble::insertarAlInicio(const string& cancion) {
		NodoHistorial* nuevo = new NodoHistorial(cancion);
		if (cabeza == nullptr) {
			cabeza = nuevo; //Lista vacia 
			cola = nuevo;
		}
		else {
			nuevo->siguiente = cabeza; // Paso 1
			cabeza->anterior = nuevo;  // Paso 2
			cabeza = nuevo;				//Paso 3
		}
		cantidad++;
	}
	...
	void HistorialDoble::insertarEnPosicion(const string& cancion, int pos) {
		...
		NodoHistorial* actual = cabeza;
		for (int i = 0; i < pos; i++) {
			actual = actual->siguiente;  //Actual es el nodo que quedara despues del nuevo
		}
		...
		previo->siguiente = nuevo; //previo -> nuevo
	...
	bool HistorialDoble::eliminarUltima(const string& cancion) {
		...
		while (actual != nullptr) {
			...
			actual = actual->anterior; //Recorido Inverso
		}
		return false;
	}
